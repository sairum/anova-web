"use strict";var sample=(function(){var precision=4;var separator=".";function multipleFTests(){let N=parseInt(document.getElementById("F_N").value);let mean=parseFloat(document.getElementById("F_avg").value);let std=parseFloat(document.getElementById("F_std").value);let n1=parseInt(document.getElementById("F_n1").value);let n2=parseInt(document.getElementById("F_n2").value);console.log(N,mean,std,n1,n2);let result=document.getElementById("F_res");result.innerHTML="";let ftests=[],v1=[],v2=[];let x,sumx,sumx2;for(let i=0;i<N;i++){x=0.0,sumx=0,sumx2=0;;for(let j=0;j<n1;j++){x=jStat.normal.sample(mean,std);sumx+=x;sumx2+=Math.pow(x,2);}
...
function multipleTTests(){let N=parseInt(document.getElementById("tt_N").value);let mean=parseFloat(document.getElementById("tt_avg").value);let std=parseFloat(document.getElementById("tt_std").value);let n=parseInt(document.getElementById("tt_n").value);console.log(N,mean,std,n);let result=document.getElementById("tt_res");result.innerHTML="";let ttests=[];let x,sumx,sumx2,x1,var1,x2,var2;for(let i=0;i<N;i++){sumx=0,sumx2=0;for(let j=0;j<n;j++){x1=jStat.normal.sample(mean,std);sumx+=x1;sumx2+=Math.pow(x1,2);}
...
let text="<h3>Multiple <em>t</em>-tests</h3>";text+="<textarea cols=\"20\" rows=\"10\" id=\"tt_results\">";for(let i=0;i<N;i++)text+=ttests[i].toFixed(precision)+"\n";text+="</textarea>";result.innerHTML=text;result.style.display="inline-block";document.getElementById("tt_sep").disabled=false;}
...
function setSTD(tagid){let s=document.getElementById(tagid+"_var").value;document.getElementById(tagid+"_std").value=Math.sqrt(s).toFixed(precision);}
function setVar(tagid){let s=document.getElementById(tagid+"_std").value;document.getElementById(tagid+"_var").value=Math.pow(s,2).toFixed(precision);}
...
function tTest(){let diff=parseInt(document.getElementById("t_diff").value);let mean=parseFloat(document.getElementById("t_avg").value);let std=parseFloat(document.getElementById("t_std").value);let n=parseInt(document.getElementById("t_n").value);let result=document.getElementById("t_res");result.innerHTML="";let stats=document.getElementById("t_stats");stats.innerHTML="";let s1=[],s2=[];let x,sumx,sumx2,x1,var1,x2,var2;sumx=0,sumx2=0;for(let i=0;i<n;i++){x=jStat.normal.sample(mean,std);sumx+=x;sumx2+=Math.pow(x,2);s1.push(x);}
...
x2=sumx/n;var2=(sumx2-Math.pow(sumx,2)/n)/(n-1);let text="<h3>Population 1</h3>";text+="<textarea cols=\"20\" rows=\"10\" id=\"t_results1\">";for(let i=0,ln=s1.length;i<ln;i++)text+=s1[i].toFixed(precision)+"\n";text+="\n";text+="</textarea>";text+="<h3>Population 2</h3>";text+="<textarea cols=\"20\" rows=\"10\" id=\"t_results2\">";for(let i=0,ln=s2.length;i<ln;i++)text+=s2[i].toFixed(precision)+"\n";text+="</textarea>";result.innerHTML=text;result.style.display="inline-block";let t=Math.abs(x1-x2)/Math.sqrt(var1/n+var2/n);let p1=1-jStat.studentt.cdf(t,2*(n-1)),p2=p1*2;text="<h3><em>t</em>-test</h3>";text+="<textarea cols=\"20\" rows=\"10\" id=\"t_ttest\">";text+="t statistic = "+t.toFixed(precision)+"\n";text+="p (two tailed) = "+(p2).toFixed(precision)+"\n";text+="p (one tailed) = "+(p1).toFixed(precision)+"\n";text+="</textarea>";stats.innerHTML=text;stats.style.display="inline-block";document.getElementById("t_sep").disabled=false;}
return{setAccordion:setAccordion,reset:reset,sampleNormal:sampleNormal,sampleNormalNTimes:sampleNormalNTimes,setVar:setVar,setSTD:setSTD,setPrecision:setPrecision,switchDecSep:switchDecSep,multipleFTests:multipleFTests,multipleTTests:multipleTTests,tTest:tTest}})();document.addEventListener("DOMContentLoaded",function(){sample.setAccordion();});