...
var ui=(function(){'use strict';var factors=[],terms=[],combins=[],recoded=[],partial=[],effects=[];function select(name){let tabcontent=document.getElementsByClassName("tabcontent");for(let i=0;i<tabcontent.length;i++){if(tabcontent[i].id==name)tabcontent[i].style.display="block";else tabcontent[i].style.display="none";}}
...
let elem=document.getElementById("result");elem.innerHTML=text;elem=document.getElementById("download");elem.disabled=false;}
function separator(){let res=document.getElementById("result").value;console.log(res);let sep=document.getElementById("separator").value;if(sep==".")res=res.replace(/\,/g,".");else res=res.replace(/\./g,",");document.getElementById("result").value=res;}
