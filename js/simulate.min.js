...
newterms.push(f.name);for(let i=0;i<terms.length;i++){if(interacts(nestedin.value,terms[i]))
...
factors.push(f);computeLevels(0);recodeNestedFactors();setupLevels(fnum);fname.value="";levels.value=2;ftype.value='fixed';nestedin.value='-';let elem=document.getElementById("generate");elem.disabled=false;}else{alert('Factor name: "'+name+'" already exits!\n'+'Please, enter a different factor name.')}}else{alert('Bad factor name: "'+name+'"\n'+'(The name is empty or contains illegal\n'+'characters such as *, ?, etc.)\n\n'+'Please, enter an alpha-numeric factor name.')}}
...
let elem=document.getElementById("result");elem.value=text;elem=document.getElementById("download");elem.disabled=false;document.getElementById("separator").disabled=false;}
...
let t=document.getElementById('tabs');let r=document.getElementById('tresults');let b=document.createElement('a');b.name=f.name;b.className='tabs';b.href="#!";b.innerHTML=f.name;b.onclick=function(){select(f.name)};t.insertBefore(b,r);let cts=document.getElementById('tab-contents');let res=document.getElementById('results');let d=document.createElement('div');d.className='tabcontent';d.id=f.name;console.log(f,combins,recoded);let text='<h3>Labels for levels of Factor '+f.name+'</h3><table>';for(let i=0;i<f.true_levels;i++){text+='<tr><td><b>Level '+(i+1).toString();if(f.nested){text+='</b> nested in ';let lst=[];for(let j=0;j<f.nestedin.length;j++){lst.push(' Level '+factors[f.nestedin[j]].lcodes[combins[i][j]]+' of Factor '+factors[f.nestedin[j]].name);}
text+=lst.join(' and ');}
text+='</td><td><input type="text" class="label" value="'+i.toString()+'"';text+=' onchange="ui.label(this)" ';text+=' id="flabel.'+fnum+'.'+i.toString()+'"></td></tr>';}
