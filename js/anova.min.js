...
partials=[];terms=[];mcomps=[];corrected_df=0;replicates=0;total={df:0,ss:0};residual={name:"Error",df:0,ss:0};nesting=false;displayData();if(computePartials())buildTerms();}
...
let h=document.getElementsByClassName("tabcontent");for(let i=0,len=h.length;i<len;i++)h[i].innerHTML="";partials=[];terms=[];corrected_df=0;replicates=0;total={df:0,ss:0};residual={name:"Error",df:0,ss:0};nesting=false;displayData();if(computePartials())buildTerms();}
...
rejection_level.toString()+'" min="0.00000" max="0.999999" step="0.01" onchange="anova.setAlpha()"/></p>'+'</div>';let d=document.getElementById('analysis');d.innerHTML=text;selectTab('analysis');}
...
table+='<td>'+terms[i].average[j].toString()+'</td>';let n=parseInt(terms[i].n[j]);table+='<td>'+n.toString()+'</td>';let std=0,variance=0;if(n>1)std=(terms[i].sumx2[j]-Math.pow(terms[i].sumx[j],2)/n)/(n-1);variance=Math.pow(std,2);table+='<td>'+std.toString()+'</td>';table+='<td>'+variance.toString()+'</td>';table+='</tr>';}
table+='</tbody></table>';}
d.innerHTML=table;}
...
factors[j].name=name;factors[j].nlevels=0;factors[j].levels=[];factors[j].nestedin=new Array(nfactors).fill(0);factors[j].depth=0;factors[j].subscript=String.fromCharCode(j+105);}
...
data.push(d);}}}
let elem=document.getElementsByClassName("tabcontent");for(let i=0,len=elem.length;i<len;i++)elem[i].innerHTML="";displayData();if(computePartials())buildTerms();}
...
elem.innerHTML=text;elem.style.display='inline-block';}else{elem.innerHTML="";elem.style.display='none';}}
...
total.df-=1;total.ss=tsumx2-Math.pow(tsumx,2)/tn;residual.orig_df=residual.df;residual.df-=corrected_df;total.orig_df=total.df;total.df-=corrected_df;replicates=partials[0].n;homogeneityTests();return true;}
...
function homogeneityTests(){let d=document.getElementById('homogen');d.innerHTML='<div class="ct">Cochran\'s test'+testCochran()+"</div>";}
