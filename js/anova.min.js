...
table+='<td class="flt">'+
terms[i].average[j].toLocaleString(undefined,fmt)+'</td>';let n=terms[i].n[j];table+='<td>'+n.toString()+'</td>';let std=0,variance=0;if(n>1){variance=(terms[i].sumx2[j]-Math.pow(terms[i].sumx[j],2)/n);variance=variance/(n-1);std=Math.sqrt(variance,2);}
table+='<td class="flt">'+
std.toLocaleString(undefined,fmt)+'</td>';table+='<td class="flt">'+
variance.toLocaleString(undefined,fmt)+'</td>';table+='</tr>';}
table+='</tbody></table>';}
d.innerHTML=table;}
