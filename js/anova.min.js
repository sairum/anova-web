...
table+='</tbody></table>';}
d.innerHTML=table;}
function displayCTRules(){let c=document.getElementById('ctrules');let table='<div class="ct"><table><thead>';table+='<tr><th>Term</th>';table+='<th>Estimates</th></tr></thead><tbody>';for(let i=0,len=terms.length-1;i<len;i++){table+='<tr><td>'+terms[i].name+'</td>';let est=[],name="",vc="&sigma",component="",maincomp="";for(let j=terms.length-2;j>=0;j--){if(terms[i].varcomp[j]>0){if((terms[j].name==='Error')||(terms[j].name==='Residual'))name='&epsilon;';else name=terms[j].name;if(terms[j].type===RANDOM)vc="&sigma;";else vc="&Sigma;";if(terms[i].varcomp[j]===1)component=vc+'<sup>2</sup><sub>'+name+'</sub>';else component=terms[i].varcomp[j].toString()+'&middot;'+vc+'<sup>2</sup><sub>'+name+'</sub>';if(i!=j)est.push(component);else maincomp=component;}}
est.push(maincomp);table+='<td>'+est.join(' + ')+'</td></tr>';}
