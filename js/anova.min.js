...
function buildTerms(){console.log('buildTerms() called');let s=Math.pow(2,nfactors);for(let i=1;i<s;i++){let temp={idx:0,name:"",codes:[],order:0,combins:1,nlevels:0,levels:[],sumx:[],sumx2:[],n:[],average:[],ss:0,df:1,SS:0,ct_codes:[],varcomp:[],MS:0,P:0,against:-1,F:0,type:FIXED};for(let j=0;j<nfactors;j++){temp.idx=i;if((i&Math.pow(2,j))){temp.codes[j]=1;temp.order++;temp.combins*=factors[j].nlevels;temp.type*=factors[j].type;}else{temp.codes[j]=0;}
temp.codes.push(0);}
terms.push(temp);}
displayTerms("Unsorted and uncorrected terms");if(getCellsSS()){terms.sort(function(a,b){return(a.order-b.order)||(a.idx-b.idx)});for(let i=0,len=terms.length-2;i<len;i++){if(terms[i].order==1){terms[i].df=factors[i].nlevels-1;}else{let m=1;for(let j=0;j<nfactors;j++){if(terms[i].codes[j]>0)m*=factors[j].nlevels-1;}
terms[i].df=m;}
...
function computeCells(){console.log('computeCells() called');let maxn=0;let dl=data.length;for(let i=0;i<dl;i++){for(let j=0,len=data[i].levels.length;j<len;j++){let l=data[i].levels[j];let k=factors[j].levels.indexOf(l);data[i].codes[j]=k;}
...
total.df-=1;total.ss=tsumx2-Math.pow(tsumx,2)/tn;residual.orig_df=residual.df;residual.df-=corrected_df;total.orig_df=total.df;total.df-=corrected_df;replicates=data[0].n;displayCells();homogeneityTests();buildTerms();}
function correctForNesting(){console.log('correctForNesting() called');if((nfactors==1)||(!nesting))return;let current=0;while(current<terms.length-2){let c=current+1;while(c<terms.length-2){if(terms[current].ss==terms[c].ss){for(let k=0;k<nfactors;k++){if((terms[c].codes[k]!=terms[current].codes[k])){terms[current].codes[k]=2;if(terms[current].order==1){factors[current].nestedin[k]=1;factors[current].type=RANDOM;}}}
...
current++;}
displayFactors(factors);displayTerms('Terms before correcting for nesting',terms);correctTermNames();for(let i=0,tl=terms.length-2;i<tl;i++){terms[i].df=1;for(let k=0;k<nfactors;k++){if(terms[i].codes[k]==1)terms[i].df*=factors[k].nlevels-1;if(terms[i].codes[k]==2)terms[i].df*=factors[k].nlevels;}
terms[i].MS=terms[i].SS/terms[i].df;}}
function correctTermNames(){console.log('correctTermNames() called');for(let i=0;i<nfactors;i++){for(let j=0;j<nfactors;j++){if(factors[i].nestedin[j]==1){factors[i].depth++;factors[i].nlevels/=factors[j].nlevels;}}}
...
function displayAverages(){console.log('displayAverages() called');let d=document.getElementById('averages');let table='';for(let i=0,len=terms.length-2;i<len;i++){table+='<h3>Averages for '+terms[i].name+'</h3>';table+='<table><thead><tr>';let cds=[...terms[i].codes];for(let j=0,jlen=cds.length;j<jlen;j++){if(cds[j]!=1)cds[j]='-';else table+='<th>'+factors[j].name+'</th>';}
...
table+='</tbody></table>';}
d.innerHTML=table;}
function displayCTRules(){console.log('displayCTRules() called');let c=document.getElementById('ctrules');let d=document.getElementById('debug');let table='<div class="ct"><table><thead>';let dbgtable='<h5>Cornfield-Tukey Rules</h5>'+table;table+='<tr><th>Term</th>';dbgtable+='<tr><th>Term</th>';for(let i=0,len=factors.length+1;i<len;i++){dbgtable+='<th>'+String.fromCharCode(i+105)+'</th>';}
...
components.push(maincomp);table+='<td>'+components.join(' + ')+'</td></tr>';dbgtable+='<td>'+components.join(' + ')+'</td></tr>';}
table+='</tbody></table></div>';dbgtable+='</tbody></table></div>';c.innerHTML=table;d.innerHTML+=dbgtable;}
function displayCells(){console.log('displayCells() called');let d=document.getElementById('debug');let table='<h5>List of Partials</h5><div><table><thead>';table+='<tr>';for(let i=0,len=factors.length;i<len;i++){table+='<th>'+factors[i].name+'</th>';}
table+='<th>n orig.</th><th>n</th><th>sumx</th>'+'<th>sumx2</th><th>ss</th></thead><tbody>';for(let i=0,len=data.length;i<len;i++){table+='<tr>';for(let j=0,l=data[i].codes.length;j<l;j++){let c=data[i].codes[j];let name=factors[j].levels[c];table+='<td>'+name+'</td>';}
table+='<td>'+data[i].n_orig.toString()+'</td>';table+='<td>'+data[i].n.toString()+'</td>';table+='<td>'+data[i].sumx.toString()+'</td>';table+='<td>'+data[i].sumx2.toString()+'</td>';table+='<td>'+data[i].ss.toString()+'</td>';table+='</tr>';}
...
function displayFactors(){console.log('displayFactors() called');let d=document.getElementById('debug');let table='<h5>Factors</h5><div><table>';table+='<thead><tr><th>Name</th><th>Subs.</th>'+'<th>Type</th><th>Levels</th><th>Levels\' Codes</th>'+'<th>Nested in</th></tr></thead><tbody>';for(let i=0,len=factors.length;i<len;i++){table+='<tr><td>'+factors[i].name+'</td>'+'<td>'+String.fromCharCode(i+105)+'</td>'+'<td>'+factors[i].type+'</td>'+'<td>'+factors[i].nlevels.toString()+'</td>'+'<td>'+factors[i].levels.toString();+'</td>'+'<td>'+factors[i].nestedin.toString();+'</td></tr>';}
table+='</tbody></table><div>';d.innerHTML+=table;}
function displayMultipleComparisons(){console.log('displayMultipleComparisons() called');let d=document.getElementById('mtests');let text='<div class="ct">'+'<h3>Multiple Comparison Tests</h3>'+'<p><input type="radio" name="test" value="none" checked>None</p>'+'<p><input type="radio" name="test" value="snk">Student-Newman-Keuls (SNK)</p>'+'<p><input type="radio" name="test" value="tukey">Tukey (HSD)</p>'+'<p>Rejection criteria (&alpha;): <input type="number" id="mtests_alpha" value="'+
...
function displayTerms(title=''){console.log('displayTerms() called');let d=document.getElementById('debug');let table='<h5>'+title+'</h5><table>';table+='<thead><tr><th>idx</th><th>Order</th><th>Name</th>'+'<th>Type</th><th>Codes</th><th>levels</th><th>combins</th>'+'<th>df</th><th>level codes</th><th>n</th><th>averages</th>'+'<th>sumx</th><th>sumx2</th><th>ss</th><th>SS</th>'+'<th>MS</th><th>F</th><th>Against</th></tr></thead><tbody>';let a=[];let tp='';for(let i=0,len=terms.length;i<len;i++){table+='<tr><td>'+terms[i].idx.toString()+'</td>';table+='<td>'+terms[i].order.toString()+'</td>';table+='<td>'+terms[i].name+'</td>';(terms[i].type===RANDOM)?tp='RANDOM':tp='FIXED';table+='<td>'+tp+'</td>';table+='<td>'+terms[i].codes.toString();+'</td>';table+='<td>'+terms[i].nlevels.toString()+'</td>';table+='<td>'+terms[i].combins.toString()+'</td>';table+='<td>'+terms[i].df.toString()+'</td>';a=terms[i].levels.slice();table+='<td>'+a.join(' : ')+'</td>';a=terms[i].n.slice();table+='<td>'+a.join(' : ')+'</td>';a=terms[i].average.slice();for(let j=0,l=a.length;j<l;j++)a[j]=a[j].toFixed(3);table+='<td>'+a.join(' : ')+'</td>';a=terms[i].sumx.slice();for(let j=0,l=a.length;j<l;j++)a[j]=a[j].toFixed(3);table+='<td>'+a.join(' : ')+'</td>';a=terms[i].sumx2.slice();for(let j=0,l=a.length;j<l;j++)a[j]=a[j].toFixed(3);table+='<td>'+a.join(' : ')+'</td>';table+='<td>'+terms[i].ss.toFixed(3)+'</td>';table+='<td>'+terms[i].SS.toFixed(3)+'</td>';table+='<td>'+terms[i].MS.toFixed(3)+'</td>';table+='<td>'+terms[i].F.toFixed(3)+'</td>';table+='<td>'+terms[i].against+'</td>';}
table+='</tbody></table>';d.innerHTML+=table;}
function getCellsSS(){console.log('getCellsSS() called');let tl=terms.length,dl=data.length;for(let i=0;i<tl;i++){let c=terms[i].codes;let nm=[];for(let j=0,l=c.length;j<l;j++){if(c[j]==1)nm.push(factors[j].name);}
...
function homogeneityTests(){console.log('homogeneityTests() called');let d=document.getElementById('homogen');d.innerHTML='<div class="ht"><h2>Cochran\'s test</h2>'+
...
function openDataFile(){console.log('openDataFile() called');let selectedFile=document.getElementById('loadFile').files[0];if(typeof(selectedFile)==='undefined')return;let textType=/text.*/;if(selectedFile.type.match(textType)){filename=selectedFile.name;let h=document.getElementById('filename');h.innerHTML='Current selected file is <b>'+filename+'</b>';resetAnalysis();let reader=new FileReader();reader.onload=function(e){let header=true;let text=reader.result;let lines=text.split('\n');for(let i=0,len=lines.length;i<len;i++){let li=lines[i].trim();if((li[0]!=='#')&&(li.length!==0)){li=li.split(/[\s\t]+/);if(header){nfactors=li.length-1;for(let j=0;j<nfactors;j++){factors[j]={};let name=li[j];if(name.endsWith('*')){factors[j].type=RANDOM;name=name.slice(0,name.length-1);}else{factors[j].type=FIXED;}
factors[j].name=name;factors[j].orig_name=name;factors[j].nlevels=0;factors[j].levels=[];factors[j].nestedin=new Array(nfactors).fill(0);factors[j].depth=0;}
...
displayFactors();displayData();computeCells();selectTab('analysis');}
...
function resetAnalysis(){console.log('resetAnalysis() called');let elems=document.getElementsByClassName('tabcontent')
...
document.addEventListener('DOMContentLoaded',function(){let t=document.getElementById('tab-contents');let d=document.createElement('div');d.className='tabcontent';d.id='debug';t.appendChild(d);t=document.getElementById('tabs');d=document.createElement('a');d.name='debug';d.className='tabs';d.href='#!';d.onclick=function(){selectTab('debug');};d.innerHTML='Debug';t.appendChild(d);anova.setSettings();let b=document.getElementsByClassName('tabcontent');for(let i=0;i<b.length;i++)b[i].style.display='none';let s=document.getElementById('settings');s.style.display='none';document.getElementById('openFile').onclick=function(){document.getElementById('loadFile').click()};document.getElementById('loadFile').onchange=function(){anova.openDataFile();};document.getElementById('activate_settings').onclick=function(){anova.displaySettings();};});