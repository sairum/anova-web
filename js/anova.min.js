"use strict";var anova=(function(){const RANDOM=0;const FIXED=1;var data=[];var nfactors=0;var factors=[];var partials=[];var terms=[];var mcomps=[];var replicates=0;var corrected_df=0;var total={df:0,ss:0};var residual={name:"Error",df:0,ss:0};var nesting=false;var max_value=Number.MIN_SAFE_INTEGER;var min_value=Number.MAX_SAFE_INTEGER;var rejection_level=0.05;var mt_rejection_level=0.05;var filename="";function displayCTRules(){let c=document.getElementById('ctrules');let table='<div class="ct"><table><thead>';table+='<tr><th>Term</th>';table+='<th>Estimates</th></tr></thead><tbody>';for(let i=0,len=terms.length-1;i<len;i++){table+='<tr><td>'+terms[i].name+'</td>';let est=[],name="";for(let j=terms.length-2;j>=0;j--){if(terms[i].varcomp[j]>0){if((terms[j].name==='Error')||(terms[j].name==='Residual'))name='&epsilon;';else name=terms[j].name;if(terms[i].varcomp[j]===1)est.push('&sigma;<sup>2</sup><sub>'+name+'</sub>');else est.push(terms[i].varcomp[j].toString()+'*&sigma;<sup>2</sup><sub>'+name+'</sub>');}}
...
factors[j].name+="("+nm.join("*")+")";}else{let j=nfl[i].codes.indexOf(1);let k=nfl[i].index;if(j!=-1)factors[k].name+="("+factors[j].name+")";}}
...
data.push(d);}}}
let elem=document.getElementsByClassName("tabcontent");for(let i=0,len=elem.length;i<len;i++)elem[i].innerHTML="";displayData();computePartials();}
...
let te={idx:tl,name:residual.name,codes:new Array(nfactors+1).fill(1),order:terms[tl-1].order+1,combins:0,nlevels:0,levels:[],sumx:[],sumx2:[],n:[],average:[],ss:0,df:residual.df,SS:residual.ss,ct_codes:new Array(nfactors+1).fill(1),varcomp:[],MS:0,P:0,against:-2,F:0};terms.push(te);let tt={idx:tl+1,name:"Total",codes:new Array(nfactors+1).fill(1),order:terms[tl].order+1,combins:0,nlevels:0,levels:[],sumx:[],sumx2:[],n:[],average:[],ss:0,df:total.df,SS:total.ss,ct_codes:[],varcomp:[],MS:0,P:0,against:-2,F:0};terms.push(tt);return true;}
...
function buildTerms(){let s=Math.pow(2,nfactors);for(let i=1;i<s;i++){let temp={idx:0,name:"",codes:[],order:0,combins:1,nlevels:0,levels:[],sumx:[],sumx2:[],n:[],average:[],ss:0,df:1,SS:0,ct_codes:[],varcomp:[],MS:0,P:0,against:-1,F:0};for(let j=0;j<nfactors;j++){temp.idx=i;if((i&Math.pow(2,j))){temp.codes[j]=1;temp.order++;temp.combins*=factors[j].nlevels;}else{temp.codes[j]=0;}
temp.codes.push(0);}
terms.push(temp);}
...
document.addEventListener('DOMContentLoaded',function(){let b=document.getElementsByClassName('tabcontent');for(let i=0;i<b.length;i++)b[i].style.display="none";document.getElementById("openFile").onclick=function(){document.getElementById("loadFile").click()};document.getElementById("loadFile").onchange=function(){anova.open();};});