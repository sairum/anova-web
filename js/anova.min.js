"use strict";var anova=(function(){const RANDOM=0;const FIXED=1;var data=[];var nfactors=0;var factors=[];var partials=[];var terms=[];var mcomps=[];var replicates=0;var corrected_df=0;var total={df:0,ss:0};var residual={name:"Error",df:0,ss:0};var nesting=false;var max_value=Number.MIN_SAFE_INTEGER;var min_value=Number.MAX_SAFE_INTEGER;var rejection_level=0.05;var mt_rejection_level=0.05;var filename="";function buildMultipleComparisons(){mcomps=[];for(let t=0,tln=terms.length;t<tln;t++){if((terms[t].P<rejection_level)&&(terms[t].nlevels>0)&&(terms[t].order<4)&&(terms[t].against!==-1)){for(let i=0,fl=factors.length;i<fl;i++){if((terms[t].codes[i]===1)&&(factors[i].type===FIXED)){let tgt={fcode:i};tgt.fname=factors[i].name;tgt.term=terms[t].name;tgt.df_against=terms[terms[t].against].df;tgt.ms_against=terms[terms[t].against].MS;tgt.averages=[];if(terms[t].order===1){tgt.type='factor';tgt.averages[tgt.term]=[];for(let j=0,jl=terms[t].average.length;j<jl;j++){let lv=terms[t].levels[j].split(',')[i];let ln=factors[i].levels[lv];let avg=terms[t].average[j];let n=terms[t].n[j];let std=0;if(n>1)std=(terms[t].sumx2[j]-Math.pow(terms[t].sumx[j],2)/n)/(n-1);tgt.averages[tgt.term].push({level:ln,average:avg,n:n,std:std});}
tgt.averages[tgt.term].sort((a,b)=>(a.average>b.average)?1:-1);mcomps.push(tgt);}else{tgt.type='interaction';for(let j=0,jl=terms[t].levels.length;j<jl;j++){let levs=terms[t].levels[j].split(',');let lv=levs[i];let ln=factors[i].levels[lv];for(let k=0,kl=factors.length;k<kl;k++){if((terms[t].codes[k]!=1)||(k==i))levs[k]="-";}
let avg=terms[t].average[j];let n=terms[t].n[j];let std=0;if(n>1)std=(terms[t].sumx2[j]-Math.pow(terms[t].sumx[j],2)/n)/(n-1);let codes=levs.join();let c=tgt.averages.hasOwnProperty(codes)?tgt.averages[codes]:-1;if(c==-1)tgt.averages[codes]=[];tgt.averages[codes].push({level:ln,average:avg,n:n,std:std});tgt.averages[codes].sort((a,b)=>(a.average>b.average)?1:-1);}
mcomps.push(tgt);}}}}}}
...
let e=terms.length-2;terms[e].MS=terms[e].SS/terms[e].df;displayTerms("List of Terms");correctForNesting();displayFactors();computeCTRules();displayTerms("List of Corrected Terms");displayAverages();buildMultipleComparisons();displayMultipleComparisons();displayANOVA();}}
...
tl=terms.length;for(let i=0;i<tl-2;i++){for(let j=tl-2;j>=0;j--){let found=true;if(i!=j){for(let k=0,vl=terms[i].varcomp.length;k<vl;k++){if(k!=i){if(terms[i].varcomp[k]!=terms[j].varcomp[k]){found=false
break;}}}}else{found=false;}
if(found){terms[i].against=j;terms[i].F=terms[i].MS/terms[j].MS;terms[i].P=1-jStat.centralF.cdf(terms[i].F,terms[i].df,terms[j].df);break;}else{terms[i].against=-1;terms[i].F=NaN;terms[i].P=NaN;}}}
displayCTRules();}
...
total.df-=1;total.ss=tsumx2-Math.pow(tsumx,2)/tn;residual.orig_df=residual.df;residual.df-=corrected_df;total.orig_df=total.df;total.df-=corrected_df;replicates=partials[0].n;displayPartials();homogeneityTests();buildTerms();}
...
text+="</tr>";}
text+='</tbody></table></div>';text+='<div class="ct"><p>Rejection criteria (&alpha;):</p>'+'<p><input type="number" id="anova_alpha" value="'+
rejection_level.toString()+'" min="0.00000" max="0.999999" step="0.1" onchange="anova.setAlpha()"/></p>'+'</div>';let d=document.getElementById('analysis');d.innerHTML=text;selectTab('analysis');}
...
table+='<td>'+terms[i].average[j].toString()+'</td>';let n=parseInt(terms[i].n[j]);table+='<td>'+n.toString()+'</td>';let std=0,variance=0;if(n>1)variance=(terms[i].sumx2[j]-Math.pow(terms[i].sumx[j],2)/n)/(n-1);std=Math.sqrt(variance,2);table+='<td>'+std.toString()+'</td>';table+='<td>'+variance.toString()+'</td>';table+='</tr>';}
table+='</tbody></table>';}
d.innerHTML=table;}
function displayCTRules(){let c=document.getElementById('ctrules');let d=document.getElementById("debug");let table='<div class="ct"><table><thead>';let dbgtable='<h5>Cornfield-Tukey Rules</h5>'+table;table+='<tr><th>Term</th>';dbgtable+='<tr><th>Term</th>';for(let i=0,len=factors.length+1;i<len;i++){dbgtable+='<th>'+String.fromCharCode(i+105)+'</th>';}
for(let i=terms.length-2;i>=0;i--){dbgtable+='<th>'+terms[i].name+'</th>';}
table+='<th>Estimates</th></tr></thead><tbody>';dbgtable+='<th>Estimates</th></tr></thead><tbody>';for(let i=0,len=terms.length-1;i<len;i++){table+='<tr><td>'+terms[i].name+'</td>';dbgtable+='<tr><td>'+terms[i].name+'</td>';for(let j=0,len=factors.length+1;j<len;j++){dbgtable+='<td>'+terms[i].ct_codes[j].toString()+'</td>';}
for(let j=terms.length-2;j>=0;j--){dbgtable+='<td>'+terms[i].varcomp[j].toString()+'</td>';}
let est=[],name="",vc="&sigma";for(let j=terms.length-2;j>=0;j--){if(terms[i].varcomp[j]>0){if((terms[j].name==='Error')||(terms[j].name==='Residual'))name='&epsilon;';else name=terms[j].name;if(terms[j].type===RANDOM)vc="&sigma;";else vc="&Sigma;";if(terms[i].varcomp[j]===1)est.push(vc+'<sup>2</sup><sub>'+name+'</sub>');else est.push(terms[i].varcomp[j].toString()+'&middot;'+vc+'<sup>2</sup><sub>'+name+'</sub>');}}
table+='<td>'+est.join(' + ')+'</td></tr>';dbgtable+='<td>'+est.join(' + ')+'</td></tr>';}
table+='</tbody></table></div>';dbgtable+='</tbody></table></div>';c.innerHTML=table;d.innerHTML+=dbgtable;}
function displayData(){let tb=document.getElementById("data");let table='<div class="ct">'+'<h3>Transformations</h3>'+'<p><input type="radio" name="transf" value="none" checked>None</p>'+'<p><input type="radio" name="transf" value="sqrt">&radic;X</p>'+'<p><input type="radio" name="transf" value="sqrt3">&#8731;X</p>'+'<p><input type="radio" name="transf" value="sqrt4">&#8732;X</p>'+'<p><input type="radio" name="transf" value="log">Log(X+1)</p>'+'<p><input type="radio" name="transf" value="ln">Ln(X+1)</p>'+'<p><input type="radio" name="transf" value="arcsin">arcsin(X)</p>'+'<p><input type="radio" name="transf" value="mult">X &times; <input type="number" id="multc" value="100"></p>'+'<p><input type="radio" name="transf" value="div">X &divide; <input type="number"  id="divc" value="100"></p>'+'<p><input type="radio" name="transf" value="pow">X&#8319; <input type="number"  id="powc" value="0.25"></p>'+'<p><button onclick="anova.transformData()">Apply</button></p>'+'<p><button onclick="anova.resetData()">Reset</button></p>'+'</div>';table+='<div class="ct">';table+='<table><thead><tr>';for(let i=0,nf=factors.length;i<nf;i++){table+='<th>'+factors[i].name+'</th>';}
...
table+='</tbody></table></div>';tb.innerHTML=table;}
function displayFactors(){let d=document.getElementById('debug');let table='<h5>Factors</h5><div><table>';table+='<thead><tr><th>Name</th><th>Subs.</th><th>Type</th><th>Levels</th><th>Levels\' Codes</th><th>Nested in</th></tr></thead><tbody>';for(let i=0,len=factors.length;i<len;i++){table+='<tr>';table+='<td>'+factors[i].name+'</td>';table+='<td>'+factors[i].subscript+'</td>';table+='<td>'+factors[i].type+'</td>';table+='<td>'+factors[i].nlevels.toString()+'</td>';table+='<td>'+factors[i].levels.toString();+'</td>';table+='<td>'+factors[i].nestedin.toString();+'</td>';table+='</tr>';}
table+='</tbody></table><div>';d.innerHTML+=table;}
...
function displayPartials(){let d=document.getElementById('debug');let table='<h5>List of Partials</h5><div><table><thead>';table+='<tr>';for(let i=0,len=factors.length;i<len;i++){table+='<th>'+factors[i].name+'</th>';}
table+='<th>n orig.</th><th>n</th><th>sumx</th><th>sumx2</th><th>ss</th></thead><tbody>';for(let i=0,len=partials.length;i<len;i++){table+='<tr>';for(let j=0,l=partials[i].codes.length;j<l;j++){let c=partials[i].codes[j];let name=factors[j].levels[c];table+='<td>'+name+'</td>';}
table+='<td>'+partials[i].n_orig.toString()+'</td>';table+='<td>'+partials[i].n.toString()+'</td>';table+='<td>'+partials[i].sumx.toString()+'</td>';table+='<td>'+partials[i].sumx2.toString()+'</td>';table+='<td>'+partials[i].ss.toString()+'</td>';table+='</tr>';}
table+="</tbody></table></div>";table+='<h5>Replicates</h5><div class="contentor"><table><thead>';table+='<tr><td>Replicates</td></tr></thead><tbody><tr><td>';table+=replicates+'</td></tr></tnody></table></div>';d.innerHTML+=table;}
function displayTerms(title=''){let d=document.getElementById('debug');let table='<h5>'+title+'</h5><table>';table+='<thead><tr><th>idx</th><th>Order</th><th>Name</th><th>Type</th><th>Codes</th><th>levels</th>'
table+='<th>combins</th><th>df</th><th>level codes</th><th>n</th>';table+='<th>averages</th><th>sumx</th><th>sumx2</th><th>ss</th><th>SS</th>'
table+='<th>MS</th><th>F</th><th>Against</th></tr></thead><tbody>';let a=[];let tp="";for(let i=0,len=terms.length;i<len;i++){table+='<tr><td>'+terms[i].idx.toString()+'</td>';table+='<td>'+terms[i].order.toString()+'</td>';table+='<td>'+terms[i].name+'</td>';(terms[i].type===RANDOM)?tp='RANDOM':tp='FIXED';table+='<td>'+tp+'</td>';table+='<td>'+terms[i].codes.toString();+'</td>';table+='<td>'+terms[i].nlevels.toString()+'</td>';table+='<td>'+terms[i].combins.toString()+'</td>';table+='<td>'+terms[i].df.toString()+'</td>';a=terms[i].levels.slice();table+='<td>'+a.join(' : ')+'</td>';a=terms[i].n.slice();table+='<td>'+a.join(' : ')+'</td>';a=terms[i].average.slice();for(let j=0,l=a.length;j<l;j++)a[j]=a[j].toFixed(3);table+='<td>'+a.join(' : ')+'</td>';a=terms[i].sumx.slice();for(let j=0,l=a.length;j<l;j++)a[j]=a[j].toFixed(3);table+='<td>'+a.join(' : ')+'</td>';a=terms[i].sumx2.slice();for(let j=0,l=a.length;j<l;j++)a[j]=a[j].toFixed(3);table+='<td>'+a.join(' : ')+'</td>';table+='<td>'+terms[i].ss.toFixed(3)+'</td>';table+='<td>'+terms[i].SS.toFixed(3)+'</td>';table+='<td>'+terms[i].MS.toFixed(3)+'</td>';table+='<td>'+terms[i].F.toFixed(3)+'</td>';table+='<td>'+terms[i].against+'</td>';}
table+='</tbody></table>';d.innerHTML+=table;}
...
terms[i].name=nm.join('*');for(let j=0;j<pl;j++){let t=[];for(let k=0;k<nfactors;k++){if(c[k]===1)t.push(partials[j].codes[k]);else t.push('-');}
...
for(let j=0,nl=terms[i].nlevels;j<nl;j++){terms[i].average[j]=terms[i].sumx[j]/terms[i].n[j];terms[i].ss+=terms[i].sumx2[j]-Math.pow(terms[i].sumx[j],2)/terms[i].n[j];}
...
function testBartlett(){let k=partials.length;let N=0;for(let i=0;i<partials.length;i++)N+=partials[i].n;let pvar=0;for(let i=0;i<partials.length;i++){pvar+=(partials[i].sumx2-Math.pow(partials[i].sumx,2)/partials[i].n)/(N-k);}
let A=(N-k)*Math.log(pvar);let B=0;for(let i=0;i<partials.length;i++){B+=(partials[i].n-1)*Math.log((partials[i].sumx2-Math.pow(partials[i].sumx,2)/partials[i].n)/(partials[i].n-1));}
let C=1/(3*(k-1));let D=0;for(let i=0;i<partials.length;i++){D+=(1/(partials[i].n-1)-1/(N-k));}
let bartlett_k=(A-B)/(1+(C*D));let prob=1.0-jStat.chisquare.cdf(bartlett_k,k-1);if(prob>1)prob=1;if(prob<0)prob=0;let result="";result+="<p>Bartlett's Test for <b><i>k</i> = "+k.toString()+"</b> averages and <b>&nu; = ";result+=(k-1).toString()+"</b> degrees of freedom: <b>"+bartlett_k.toString()+"</b></p>";result+="<p>P = <b>"+prob.toString()+"</b></p>";return result;}
...
let result="";result+="<p>Cochran's Test for <b><i>k</i> = "+k.toString()+"</b> averages and <b>&nu; = ";result+=df.toString()+"</b> degrees of freedom: <b>"+cochran_C.toString()+"</b></p>";result+="<p>P = <b>"+prob.toString()+"</b></p>";let cv10=0;let cv05=0;let cv01=0;cv10=1/(1+(k-1)/(jStat.centralF.inv(1-0.10/k,df,df*(k-1))));cv05=1/(1+(k-1)/(jStat.centralF.inv(1-0.05/k,df,df*(k-1))));cv01=1/(1+(k-1)/(jStat.centralF.inv(1-0.01/k,df,df*(k-1))));result+="<p>Critical values for &alpha;</p>";result+="<p><i>0.10</i>: "+cv10.toString()+", hence ";result+=(cochran_C>cv10?"variances are heterogeneous":"variances are homogeneous");result+="</p>";result+="<p><i>0.05</i>: "+cv05.toString()+", hence ";result+=(cochran_C>cv05?"variances are heterogeneous":"variances are homogeneous");result+="</p>";result+="<p><i>0.01</i>: "+cv01.toString()+", hence ";result+=(cochran_C>cv01?"variances are heterogeneous":"variances are homogeneous");result+="</p>";return result;}
function homogeneityTests(){let d=document.getElementById('homogen');d.innerHTML='<div class="ct">Cochran\'s test'+testCochran()+"</div>";d.innerHTML+='<div class="ct">Bartlett\'s test'+testBartlett()+"</div>";}
...
header=false;}else{if(li.length!=nfactors+1){let ln=i+1;let c=li.length.toString();let ec=nfactors+1;alert('In line '+ln.toString()+' number of columns ('+c+') is different from what is expected ('+e.toString()+')');return;}
...
d.levels.push(li[j]);}
let n=+li[nfactors].replace(",",".");let a=Number.parseFloat(n);if(Number.isNaN(a)){let ln=i+1;alert('In line '+ln.toString()+' data value ('+n.toString()+') is not a valid number!');return;}else{d.value=n;d.original=n;if(n>max_value)max_value=n;if(n<min_value)min_value=n;}
data.push(d);}}}
let elem=document.getElementsByClassName("tabcontent");for(let i=0,len=elem.length;i<len;i++)elem[i].innerHTML="";displayFactors();displayData();computePartials();}
...
nfactors=0;factors=[];data=[];partials=[];terms=[];corrected_df=0;replicates=0;total={df:0,ss:0};residual={name:"Error",df:0,ss:0};nesting=false;max_value=Number.MIN_SAFE_INTEGER;min_value=Number.MAX_SAFE_INTEGER;}
...
for(let i=0;i<factors.length;i++){factors[i].name=factors[i].orig_name;factors[i].nlevels=factors[i].levels.length;factors[i].nestedin=new Array(nfactors).fill(0);factors[i].depth=0;}
partials=[];terms=[];corrected_df=0;replicates=0;total={df:0,ss:0};residual={name:"Error",df:0,ss:0};nesting=false;displayData();computePartials();}
...
function setMtAlpha(){mt_rejection_level=parseFloat(document.getElementById('mtests_alpha').value);if(rejection_level>1)rejection_level=0.9999999;if(rejection_level<0)rejection_level=0.0000001;buildMultipleComparisons();multipleTests();}
function transformData(){let t=document.getElementsByName("transf");let multc=parseFloat(document.getElementById("multc").value);let divc=parseFloat(document.getElementById("divc").value);let powc=parseFloat(document.getElementById("powc").value);max_value=Number.MIN_SAFE_INTEGER;min_value=Number.MAX_SAFE_INTEGER;let transf=0;for(let i=0;i<t.length;i++){if(t[i].checked){transf=i;break;}}
for(let i=0;i<data.length;i++){let v=data[i].value;switch(transf){case 1:if(v>=0)v=Math.sqrt(v);break;case 2:v=Math.pow(v,1/3);break;case 3:v=Math.pow(v,0.25);break;case 4:if(v>=0)v=Math.log(v+1)/Math.log(10);break;case 5:if(v>=0)v=Math.log(v+1);break;case 6:if((v>=0)&&(v<=1))v=Math.asin(v);break;case 7:v*=multc;break;case 8:if(divc!=0)v/=divc;break;case 9:v=Math.pow(v,powc);break;}
data[i].value=v;if(v>max_value)max_value=v;if(v<min_value)min_value=v;}
let h=document.getElementsByClassName("tabcontent");for(let i=0,len=h.length;i<len;i++)h[i].innerHTML="";for(let i=0;i<factors.length;i++){factors[i].name=factors[i].orig_name;factors[i].nlevels=factors[i].levels.length;factors[i].nestedin=new Array(nfactors).fill(0);factors[i].depth=0;}
partials=[];terms=[];corrected_df=0;replicates=0;total={df:0,ss:0};residual={name:"Error",df:0,ss:0};nesting=false;displayData();computePartials();}
return{setAlpha:setAlpha,setMtAlpha:setMtAlpha,open:openDataFile,resetData:resetData,transformData:transformData,multipleTests:multipleTests}})();function selectTab(name){let tabs=document.getElementsByClassName("tabs");for(let i=0,len=tabs.length;i<len;i++){if(tabs[i].name==name)tabs[i].classList.add("selected");else tabs[i].classList.remove('selected');}
...
document.addEventListener('DOMContentLoaded',function(){let t=document.getElementById('tab-contents');let d=document.createElement('div');d.className='tabcontent';d.id='debug';t.appendChild(d);t=document.getElementById('tabs');d=document.createElement('a');d.name='debug';d.className='tabs';d.href='#!';d.onclick=function(){selectTab('debug');};d.innerHTML='Debug';t.appendChild(d);let b=document.getElementsByClassName('tabcontent');for(let i=0;i<b.length;i++)b[i].style.display="none";document.getElementById("openFile").onclick=function(){document.getElementById("loadFile").click()};document.getElementById("loadFile").onchange=function(){anova.open();};});